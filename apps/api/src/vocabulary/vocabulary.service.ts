...
interface SenseRow {
  ...
  entryWikidataQid: string | null;
  position: number;
  definition: string;
  tags: string[];
  senseIdHint: string | null;
  senseWikidataQid: string | null;
...
export function buildWhereSql(
  ...
  if (dto.partOfSpeech) {
    clauses.push(Prisma.sql`e.part_of_speech = ${dto.partOfSpeech}`);
  }
  ...
  if (dto.hasPolishTranslation !== undefined) {
    clauses.push(
      dto.hasPolishTranslation
        ? Prisma.sql`EXISTS (SELECT 1 FROM translations t WHERE t.vocabulary_sense_id = s.id AND t.language = 'pl')`
        : Prisma.sql`NOT EXISTS (SELECT 1 FROM translations t WHERE t.vocabulary_sense_id = s.id AND t.language = 'pl')`,
    );
  }
  ...
  return Prisma.sql`${SENSE_SELECT} WHERE ${Prisma.join(clauses, ' AND ')}`;
}

function orderBySql(sort: VocabularySort, order: 'asc' | 'desc'): Prisma.Sql {
  const direction: 'ASC' | 'DESC' = order === 'desc' ? 'DESC' : 'ASC';
  switch (sort) {
    case 'lemma':
      return Prisma.sql`ORDER BY e.normalized_lemma ${Prisma.raw(direction)}, s.position ASC`;
    case 'frequency':
      return Prisma.sql`ORDER BY f.rank ${Prisma.raw(direction)} NULLS LAST, s.position ASC`;
    case 'position':
      return Prisma.sql`ORDER BY s.position ${Prisma.raw(direction)}`;
    case 'id':
    default:
      return Prisma.sql`ORDER BY s.id ${Prisma.raw(direction)}`;
  }
}

function buildPageSql(dto: ListVocabularyDto): Prisma.Sql {
  const offset = (dto.page - 1) * dto.limit;
  return Prisma.sql`${buildWhereSql(dto)}
...
export class VocabularyService {
  ...
  async list(dto: ListVocabularyDto): Promise<PaginatedList> {
    const [rows, counted] = await Promise.all([
      this.client.$queryRaw<SenseRow[]>(buildPageSql(dto)),
    ...
    const data: VocabularyListItem[] = rows.map((row) => ({
  ...
  async search(dto: SearchVocabularyDto): Promise<PaginatedList> {
    const listDto = new ListVocabularyDto();
  ...
  > {
    ...
    const generatedChild =
      /_(BASICS|PEOPLE|PLACES|ACTIONS|THINGS|PROBLEMS|SKILLS|EVENTS)(_[1-4])?$/;
    ...
    return rows
      .filter(
        (r) => !placeholder.test(r.name.trim()) && !generatedChild.test(r.code),
      )
      .map((r) => ({
        id: Number(r.id),
        code: r.code,
  ...
  async detail(id: number): Promise<VocabularyDetailEntry> {
    ...
    const topicRows =
      senseIds.length === 0
        ...
        : await this.client.$queryRaw<
            Array<{ vocabulary_sense_id: bigint; topic: string }>
          >`
            SELECT vocabulary_sense_id, topic
            FROM sense_topics
            WHERE vocabulary_sense_id IN (${Prisma.join(senseIds)})
          `;
    const topicsBySense = new Map<string, Array<{ code: string; name: string }>>();
        ...
        categories: [
          ...s.vocabulary_category_assignments
            ...
            .filter(
              (c) =>
                c &&
                !/^(basics|people|places|actions|things|problems|skills|events) [1-4]$/i.test(
                  c.name,
                ),
            ),
  ...
  private async loadTranslations(
    rows: SenseRow[],
  ): Promise<Map<bigint, PolishTranslation[]>> {
    if (rows.length === 0) {
      return new Map();
    }
    const ids = rows.map((r) => r.senseId);
    const results = await this.client.$queryRaw<TranslationRow[]>(
      Prisma.sql`SELECT
    ...
    const grouped = new Map<bigint, PolishTranslation[]>();
