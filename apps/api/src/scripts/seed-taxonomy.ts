/**
 * Seeds ~1000 hierarchical taxonomy nodes (Domain → Subcategory → Category)
 * and assigns current vocabulary senses by keyword match.
 *
 *   npx ts-node -P tsconfig.json src/scripts/seed-taxonomy.ts
 */
...
function leaves(prefix: string, items: Array<[string, string, string[]]>): Leaf[] {
...
function buildTaxonomy(): Domain[] {
  const spec: Array<[string, string, Array<[string, string, Array<[string, string, string[]]>]>]> = [
    ['HOME', 'Home & Housing', [
  ...
  const domains: Domain[] = spec.map(([code, name, subs]) => ({
    code,
    name,
  ...
  // Pad to ~1000 nodes with systematic topic leaves under extra domains.
  const extras = [
  ...
  extras.forEach((domain, di) => {
    ...
    // Give extras a few real keywords so they can still match.
    const seeds = [
...
async function assignLeaf(categoryId: bigint, keywords: string[]): Promise<number> {
